	...
	methods
		...
		function obj = LoadExampleCellsActivities(obj, dataFile)
			...
			if( nargin() < 2 || isempty(dataFile) )
				dataFile = '../../data/Simulated Activities/Noise & Grating Simulated Separately/Separate Noise & Grating.mat';
			end
		...
		function FR = ExampleCellsActivitiesOnCondition(obj, cellType, ecc, sf, contrast, alignEvent, timeRange)
		...
		function [LFR, time, conditions, trials, trialsIdx, idxExampleCells, idxAllCells, nExampleCells, nAllCells] = ExampleCells(obj, idxConditions, dataFolder, alignEvent, stabilize, saveFolder)
		...
		function [idxExampleCells, idxAllCells, nExampleCells, nAllCells] = GetExampleCells(obj, angularRange, dataFolder, isPlot)
			...
			if(isPlot)
				...
				centers = -100:5:100;
				data = hist( [trials.saccadeOn] - [trials.saccadeOff], centers );
				bar( centers, data, 0.9, 'b', 'LineStyle', 'none', 'displayName', 'sacOn - sacOff' );
				% data = hist( [trials.saccadeOff] - [trials.flashOn], centers );
				% bar( centers, data, 0.9, 'r', 'LineStyle', 'none', 'displayName', 'sacOff - flashOn' );
				data = hist( [trials.saccadeLand] - [trials.saccadeOff], centers );
		...
		function MeanPopulationActivity(obj)
			...
			colors = {'r', 'g', 'b'};
			for(k = 8:-1:1)
			    hAxes(k) = axes( 'Position', [0.06+0.96/4*(mod(k-1,4)), 0.08+0.92/2*(k<5), 0.96/4-0.03, 0.9/2-0.04], ...
			    ...
			    for(iEcc = size(Eccs,2) : -1 : 1)
					...
					frBG = obj.ExampleCellsActivitiesOnCondition(obj.layers(iL).name, Eccs(iEcc), 0, 0, alignEvent, t([1 end]));
					fr = obj.ExampleCellsActivitiesOnCondition(obj.layers(iL).name, Eccs(iEcc), SFs(iSF), 0.5);
					...
					h(size(Eccs,2)+2) = plot( [0 0], get(gca, 'ylim'), 'k--', 'lineWidth', 2, 'displayName', 'Saccade off' );
					h(size(Eccs,2)+3) = plot( [1 1] * mean([obj.activityParams.trials.saccadeOn] - [obj.activityParams.trials.(alignEvent)]), get(gca, 'ylim'), '--', 'color', [0.5 0.5 0.5], 'lineWidth', 2, 'displayName', 'Mean SacOn' );
		...
		function AccumulatedPopulationActivity(obj)
			...
			for(k = 8:-1:1)
			    ...
			    for(iEcc = size(Eccs,2) : -1 : 1)
					...
					frBG = obj.ExampleCellsActivitiesOnCondition(obj.layers(iL).name, Eccs(iEcc), 0, 0, alignEvent, t([1 end]));
					fr = obj.ExampleCellsActivitiesOnCondition(obj.layers(iL).name, Eccs(iEcc), SFs(iSF), 0.5, alignEvent, t([1 end]));

					frBG = cumsum(mean(frBG, 1), 2);	% accumulated population activity
	                mBG = mean(frBG, 3);
	                semBG = std(frBG, [], 3) / sqrt(size(frBG,3));

	                fr = cumsum(mean(fr, 1), 2);		% accumulated population activity
					m = mean(fr, 3);
					sem = std(fr, [], 3) / sqrt(size(fr,3));
		...
		function [tTicks, tprAccPredict, tprSegPredict] = TrialPredict(obj, classifier, contrast, alignEvent, timeRange, tStep, tWin, saveFolder)
			...
			for(iCond = size(conditions,2) : -1 : 1)
				for(iL = 4:-1:1)
					LFR{iCond,iL} = obj.ExampleCellsActivitiesOnCondition(obj.layers(iL).name, conditions(iCond).eccentricity, conditions(iCond).sf, contrast, alignEvent, timeRange);
				end
			end
						...
						switch(lower(classifier))
							case 'thresholding-uni'
								...
								if( k > 1 )
									...
									% if(tTicks(iTick) <= 50)
										thresholdAcc{iCond,iL} = prctile( y(T==0), 100 * (1 - fprFun(tTicks(iTick))) );		% set threshold at the level giving false alarm rate measured empirically
									% end
		...
		function [tTicks, accTPRm, Thresholds, ThresholdsSTD, segTPRm] = ContrastDetection(obj, classifier, CHANCE, dataFolder)
			...
			dataFolder = fullfile(dataFolder, 'figures', classifier);
			...
			if( exist( fullfile(dataFolder, 'PerformanceData.mat') ) )
				...
				for( iContrast = 1 : size(contrasts,2) )
					...
					[tTicks{iContrast}, accTPRm(:,:,:,:,iContrast), segTPRm(:,:,:,:,iContrast)] = obj.TrialPredict(classifier, contrasts(iContrast), 'saccadeOff', [0 600], [], [], fullfile(dataFolder, sprintf('Trial-Prediction Time Course Contrast=%.2f', contrasts(iContrast))));
					drawnow;
					pause(3);
					for(iL = 5:-1:1)
...
% 						saveas( gcf, fullfile( dataFolder, sprintf( 'Time Course of Trial-by-Trial Prediction with %s, %s+[%d,%d], %s.png', classifier, 'saccadeOff', 0, 500, layerNames{iL} ) ) );
				...
				tTicks = tTicks{1};
				save( fullfile(dataFolder, 'PerformanceData.mat'), 'tTicks', 'accTPRm', 'segTPRm', 'conditions' );
			end
						...
						for( iDur = 1 : nDurs )
							...
							if(fitThreshold)
								if( ~any(isnan(accTPRm(iL,iSF,iEcc,iTick,:))) )
									...
									iMin = find(accTPRm(iL,iSF,iEcc,iTick,:) <= chance, 3, 'last');
									iMax = find(accTPRm(iL,iSF,iEcc,iTick,:) == 1, 3, 'first');
									if(isempty(iMax))
					...
					if( iTick == find(time == tStart) )
						for( iEcc = 1 : size(Eccs,2) )
							for( iL = 1 : 4 )
								...
								eyeIdx = time(iTick) + trials(iTrial).(conditions(1).alignEvent) + (-10:0);
								if(iEcc == 1)
									ecc = 0.75;
								else
									ecc = Eccs(iEcc);
