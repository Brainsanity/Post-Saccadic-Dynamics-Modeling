classdef Encoder < handle
	...
	methods
		...
		function FR = AddInternalNoise(obj, FR, sbj)
			
			if(~isfield(obj.activityParams, 'internalNoise'))
				load(fullfile('../../Data/Simulated Activities', sbj, 'Noise & Grating Simulated Separately/Noise & Grating Simulated Separately.mat'));
		...
		function [LFR, time, conditions, trials, trialsIdx, idxExampleCells, idxAllCells, nExampleCells, nAllCells] = SimulateExampleCellsActivities(obj, idxConditions, dataFolder, sbj, alignEvent, stabilize, saveFolder)
			...
			for( iCond = sort(idxConditions, 'descend') )	%size(conditions,2) : -1 : 1 )
				...
				lfr = LFR(iCond,:);
				save( fullfile( dataFolder, 'Simulated Activities', sbj, saveFolder, sprintf('%s-%02d.mat', saveFolder, iCond) ), 'sbj', 'iCond', 'lfr', 'time', 'conditions', 'trials', 'trialsIdx', 'idxExampleCells', 'idxAllCells', 'nExampleCells', 'nAllCells' );
			end
		...
		function [tTicks, tprAccPredict, tprSegPredict] = TrialPredict(obj, classifier, contrast, alignEvent, timeRange, tStep, tWin, saveFolder, durOffset, withInternalNoise)
			...
			sbj = 'A014';
			if(strcmpi(classifier, 'thresholding-uni-optim_fa'))
						...
						for( iDur = 1 : nDurs )
							...
							if(fitThreshold)
								if( ~any(isnan(accTPRm(iL,iSF,iEcc,iTick,:))) )
                                    ...
                                    iMin2 = max(1, find(accTPRm(iL,iSF,iEcc,iTick,:) >= chance, 1, 'first') - 1);
                                    iMin = max(iMin, iMin2);
