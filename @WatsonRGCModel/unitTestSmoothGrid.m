function unitTestSmoothGrid()
    ...
    gridParams.lambdaMin = 1.873452;%2;     %%%%%% YB. 2020.07.10. To match maximal density of P cell. %%%%%%
    ...
    %%%%%% YB. 2020.07.06. %%%%%%
    width = mosaicFOVDegs * 1.07 * 1.2 * gridParams.micronsPerDegree;
    scaleF = sqrt(3) / 2;
    marginInRFPositions = 0.1;
    [X, Y] = meshgrid(scaleF*gridParams.lambdaMin : scaleF*gridParams.lambdaMin : width+marginInRFPositions, gridParams.lambdaMin : gridParams.lambdaMin : width+marginInRFPositions);
    Y(:,2:2:end) = Y(:,2:2:end) - 0.5*gridParams.lambdaMin;
    rfPositions = [X(:) - mean(X(:)) Y(:) - mean(Y(:))];
...
function rfPositions = generateInitialRFpositions(fovDegs, lambda, micronsPerDeg)
    ...
    % rows = 2 * radius;
    % rows = fovDegs*1.2*micronsPerDeg;           %%%%%% YB.2020.07.06. Improve efficiency. %%%%%%
    % cols = rows;
    % rfPositions = computeHexGrid(rows, cols, lambda);

    %%%%%% YB. 2020.07.06. Optimized code. %%%%%%
    width = fovDegs*1.2*micronsPerDeg;
    scaleF = sqrt(3) / 2;
    marginInRFPositions = 0.1;
    [X, Y] = meshgrid(scaleF*lambda : scaleF*lambda : width+marginInRFPositions, lambda : lambda : width+marginInRFPositions);
    Y(:,2:2:end) = Y(:,2:2:end) - 0.5*lambda;
    rfPositions = [X(:) - mean(X(:)) Y(:) - mean(Y(:))];
end
...
function hexLocs = computeHexGrid(rows, cols, lambda)
    ...
    %%%%%% YB. 2020.07.06. Optimized code. %%%%%%
    [X2, Y2] = meshgrid(scaleF*lambda : scaleF*lambda : cols+marginInRFPositions, lambda : lambda : rows+marginInRFPositions);
    Y2(:,2:2:end) = Y2(:,2:2:end) - 0.5*lambda;
    hexLocs = [X2(:) - mean(X2(:)) Y2(:) - mean(Y2(:))];
end
...
function [mRGCSpacingInMicrons, mRGCDensityInMicrons, eccentricitiesInMicrons] = mRGCSpacingFunctionDirect2(rfPositions, eccentricitySamplesNum, whichEye)
    ...
    %%%%%% YB. 2020.07.06. Obtain spacing directly instead of interpolation. %%%%%%
    requestedEccentricities = sqrt( sum(rfPositions.^2,2) )/1000;
    requestedAngles = atan2d(rfPositions(:,1),rfPositions(:,2));

    for k = 1:4
        [~,meridianmRGCRFDensity(k,:)] =  WatsonRGCModelObj.mRGCRFSpacingAndDensityAlongMeridian(requestedEccentricities, WatsonRGCModelObj.enumeratedMeridianNames{k}, ...
            'mm', 'mm^2', ...
            'adjustForISETBioConeDensity', false, ...
            'subtype', 'ONOFF');
    end
    requestedAngles(requestedAngles<0) = requestedAngles(requestedAngles<0) + 360;
    mRGCRFDensity2D = sqrt( ( rfPositions(:,1) .* meridianmRGCRFDensity((1:4:end)'+(rfPositions(:,1)<0)*2) ).^2 + ( rfPositions(:,2) .* meridianmRGCRFDensity((1:4:end)'+(rfPositions(:,2)>0)+(rfPositions(:,2)<0)*3) ).^2 ) ./ eccentricitiesInMicrons;
    mRGCRFDensity2D(isnan(mRGCRFDensity2D)) = meridianmRGCRFDensity(1,isnan(mRGCRFDensity2D));
    mRGCDensityInMicrons = mRGCRFDensity2D / 1e6;
    mRGCSpacingInMicrons = WatsonRGCModelObj.spacingFromDensity(mRGCDensityInMicrons);
end
