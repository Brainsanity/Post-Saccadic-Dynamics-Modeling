function [mRGCRFDensity2D, meridianDensities, spatialSupport, xLabelString, yLabelString, ...
    ...
    %%%%%% YB. 2020.07.05. Replace the above angular interpolation with Equation 15 in Watson (2014) paper. %%%%%%
    requestedAngles(requestedAngles<0) = requestedAngles(requestedAngles<0) + 360;
    mRGCRFDensity2D = sqrt( ( eccX .* meridianmRGCRFDensity((1:4:end)'+(eccX<0)*2) ).^2 + ( eccY .* meridianmRGCRFDensity((1:4:end)'+(eccY>0)+(eccY<0)*3) ).^2 ) ./ requestedEccentricities;
