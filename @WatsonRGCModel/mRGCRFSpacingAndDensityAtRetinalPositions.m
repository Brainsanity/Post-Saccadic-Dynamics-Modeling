...
% right eye.
    ...
    %%%%%% YB. 2020.07.05. Equation 15 in Watson (2014) paper. %%%%%%
    % eccAngles(eccAngles<0) = eccAngles(eccAngles<0) + 360;
    % mRGCRFDensities = sqrt( ( rfPositions(:,1) .* meridianRFDensities((1:4:end)'+(rfPositions(:,1)<0)*2) ).^2 + ( rfPositions(:,2) .* meridianRFDensities((1:4:end)'+(rfPositions(:,2)>0)+(rfPositions(:,2)<0)*3) ).^2 ) ./ eccMags;
    mRGCRFDensities(isnan(mRGCRFDensities)) = meridianRFDensities(1,isnan(mRGCRFDensities));
