classdef TemporalRF < handle
	...
	methods
		...
		function obj = TemporalRF()
			...
			paperParams('POnCenter')	= struct(	'A',		[19.41,		128.10,		67.59,		63.68,		33.06,		0.52], ...	% spikes / (s * unit contrast); mean +- std
			...
			paperParams('POnSurround')	= struct( 	'A',		[29.67,		519.63,		49.98,		126.46,		193.23,		1.53], ...	% spikes / (s * unit contrast); mean +- std
										            'D',		[2.5,		4,			3.5,		3,47,		0.51,		0.15] / 1000, ...	% s, delay of transmission from the optic chiasm to the LGN
			...
			paperParams('POffCenter') 	= struct(	'A',		[29.58,		114.05,		52.95,		68.96,		25.31,		0.37], ...
			...
			paperParams('POffSurround')	= struct( 	'A',		[12.54,		420.20,		23.91,		76.07,		139.97,		1.84], ...	% spikes / (s * unit contrast); mean +- std
			...
			paperParams('nPOnCenter') = 13;
			paperParams('nPOffCenter') = 11;
			paperParams('nPOnSurround') = 6;
			paperParams('nPOffSurround') = 8;


			if false
			% Benardete & Kaplan, JOP, 1999		(values for D taken from Benardete & Kaplan, Visual Neuroscience, 1997, I)
			%	min, max, median, mean, s.d.
			paperParams('POnCenter')	= struct(	'A',		[7.48,		111.42,		13.7,		22.87,		27.11], ...	% spikes / (s * unit contrast); mean +- std
			...
			paperParams('POnSurround')	= struct( 	'A',		[2.41,		43.12,		10.44,		12.63,		9.69], ...	% spikes / (s * unit contrast); mean +- std
										            'D',		[2.5,		4,			3.5,		3,47,		0.51] / 1000, ...	% s, delay of transmission from the optic chiasm to the LGN
			...
			paperParams('POffCenter') 	= struct(	'A',		[10.15,		20.29,		14.20,		14.27,		3.34], ...
			...
			paperParams('POffSurround')	= struct( 	'A',		[8.27,		65.13,		10.11,		18.04,		20.85], ...	% spikes / (s * unit contrast); mean +- std
			...
			paperParams('nPOnCenter') = 13;
			paperParams('nPOffCenter') = 7;
			paperParams('nPOnSurround') = 13;
			paperParams('nPOffSurround') = 7;
			end
		...
		function params = SynthesizeParams( obj, cellType, nCells )
			...
			switch cellType
				case {'POn', 'POff'}
					...
					[params.centerA] = deal(obj.paperParams([cellType, 'Center' ]).A(3));
					[params.centerD] = deal(obj.paperParams([cellType, 'Center' ]).D(3));
					[params.centerN_L] = deal(obj.paperParams([cellType, 'Center' ]).N_L(3));
					[params.centerTau_L] = deal(obj.paperParams([cellType, 'Center' ]).Tau_L(3));
					[params.centerH_S] = deal(obj.paperParams([cellType, 'Center' ]).H_S(3));
					[params.centerTau_H] = deal(obj.paperParams([cellType, 'Center' ]).Tau_H(3));

					[params.surroundA] = deal(obj.paperParams([cellType, 'Surround' ]).A(3));
					[params.surroundD] = deal(obj.paperParams([cellType, 'Surround' ]).D(3));
					[params.surroundN_L] = deal(obj.paperParams([cellType, 'Surround' ]).N_L(3));
					[params.surroundTau_L] = deal(obj.paperParams([cellType, 'Surround' ]).Tau_L(3));
					[params.surroundH_S] = deal(obj.paperParams([cellType, 'Surround' ]).H_S(3));
					[params.surroundTau_H] = deal(obj.paperParams([cellType, 'Surround' ]).Tau_H(3));
				
				case {'MOn', 'MOff'}
					params(nCells).A = [];
					[params.A] = deal(obj.paperParams(cellType).A(3));
					[params.D] = deal(obj.paperParams(cellType).D(3));
					[params.N_L] = deal(obj.paperParams(cellType).N_L(3));
					[params.Tau_L] = deal(obj.paperParams(cellType).Tau_L(3));
					[params.H_S] = deal(obj.paperParams(cellType).H_S(3));
					[params.Tau_0] = deal(obj.paperParams(cellType).Tau_0(3));
					[params.C_half] = deal(obj.paperParams(cellType).C_half(3));
